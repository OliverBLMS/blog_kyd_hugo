...
`Blogdown` is a R package to generate static websites based on [R Markdown](http://rmarkdown.rstudio.com/) and [Hugo](https://gohugo.io/), a static web engine. To be honost: It's **fantastic**!  
...
There is a big difference between `blogdown::server_site()`, `blogdown::hugo_build()` and `blogdown::build_site()`:

  * `blogdown::server_site()` is for local use only. It will generate `html` files of your new/adjusted (r)markdown posts.
  * `blogdown::hugo_build()` builds the `public` skeleton, but will not generate new `html` post compared to `blogdown::server_site()`
  * `blogdown::build_site()` builds the **whole** website for publishing 
